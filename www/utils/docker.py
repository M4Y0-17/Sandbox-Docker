...
def generate_docker_compose(template_path, port, password, eml_file_path=None):
    with open(template_path, 'r') as file:
        template = file.read()

    config = template.replace("${PORT}", str(port)).replace("${PASSWORD}", password)

    if eml_file_path:
        config = config.replace("${EML_FILE_PATH}", eml_file_path)
        # Añade la ruta del archivo .eml como una variable de entorno
        config = config.replace("${EML_FILE_ENV}", f'- EML_FILE_PATH={eml_file_path}')
    else:
        config = config.replace("${EML_FILE_PATH}", "")
        config = config.replace("${EML_FILE_ENV}", "")
...
def delete_container(container_name, max_retries=5, wait_seconds=5):
    ...
    if container_deleted:
        with current_app.app_context():
            ...
            if container_entry:
                # Aquí es donde deberías eliminar el directorio
                print(container_entry.directory_path)
                if container_entry.directory_path and os.path.isdir(container_entry.directory_path):
                    try:
                        shutil.rmtree(container_entry.directory_path)
                        # Para directorios no vacíos, considera usar shutil.rmtree(container_entry.directory_path)
                        print(f"Directorio {container_entry.directory_path} eliminado con éxito.")
                    except Exception as e:
                        print(f"No se pudo eliminar el directorio {container_entry.directory_path}: {e}")
